...
export async function POST(request: NextRequest) {
  try {
    ...
    if (questError || !twitterFollowQuest) {
      return NextResponse.json({ error: 'Twitter follow quest not found' }, { status: 404 });
    }
    ...
    if (insertError) {
      console.error('❌ Complete Follow Quest - Failed to insert completion:', insertError);
    ...
    console.log('✅ Complete Follow Quest - Quest completed successfully:', newCompletion);
