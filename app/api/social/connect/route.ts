...
export async function POST(request: NextRequest) {
  try {
    ...
    const verificationToken = `${platform}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    if (insertError) {
      console.log('❌ Social Connect - Failed to create pending connection:', insertError)
      throw insertError
    }

    console.log('✅ Social Connect - Pending connection created:', pendingConnection)
